...
struct AXIUtils {
  ...
  void set_burst_fixed(ChipletPayload *transaction,
                       unsigned int data_length) const {
    unsigned int beats = (data_length + axi_width_bytes - 1) / axi_width_bytes;
    ...
    transaction->set_axi_length(beats - 1);
    transaction->set_axi_size(static_cast<uint8_t>(std::log2(axi_width_bytes)));
    transaction->set_axi_burst(0);
  }

  void set_burst_incr(ChipletPayload *transaction,
                      unsigned int data_length) const {
    unsigned int beats = (data_length + axi_width_bytes - 1) / axi_width_bytes;
    ...
    transaction->set_axi_length(beats - 1);
    transaction->set_axi_size(static_cast<uint8_t>(std::log2(axi_width_bytes)));
    transaction->set_axi_burst(1);
  }

  void set_burst_wrap(ChipletPayload *transaction,
                      unsigned int data_length) const {
    unsigned int beats = (data_length + axi_width_bytes - 1) / axi_width_bytes;
