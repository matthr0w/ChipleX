...
int main() {
  ...
  const int original_compressed_data_size =
      resized_data_size - sizeof(unsigned int);
  unsigned int original_data_size;
  memcpy(&original_data_size, compressed_data, sizeof(unsigned int));

  // allocate buffer
  char *const decompressed_data = malloc(original_data_size);

  // decompress data
  const int decompressed_data_size = LZ4_decompress_safe(
      compressed_data + sizeof(unsigned int), decompressed_data,
      original_compressed_data_size, original_data_size);

  uint64_t end_cycles = read_cycles();

  printf("Cycles: %lu\n", end_cycles - start_cycles);

  if (memcmp(str, decompressed_data, str_size) == 0)
    printf("Validation done. Output:\n%s\n", decompressed_data);
