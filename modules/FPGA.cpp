...
FPGA::FPGA(sc_module_name name)
    : sc_module(name), fpga_id(0), axi_utils(axi_width / 8),
      axi_interconnect("AXI_Interconnect", fpga_id, num_axi_managers,
                       num_axi_subordinates, axi_clk_cycle,
                       axi_arbitration_delay),
      axi_manager_core("AXI_M_Core", axi_width, axi_clk_cycle),
...
void FPGA::initialize() {
  ...
  axi_manager_core.isocket.bind(axi_interconnect.tsockets[0]);
  axi_manager_interconnect.isocket.bind(axi_interconnect.tsockets[1]);
