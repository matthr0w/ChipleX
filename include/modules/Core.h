...
SC_MODULE(Core) {
  private:
	...
	const sc_time     irq_hw_delay;
	const unsigned    irq_sw_delay;
  ...
  private:
	...
	std::shared_ptr<RequestHandle> write_internal(
	    uint32_t request_id, uint8_t src_module, uint8_t dst_chiplet, uint8_t dst_module, uint32_t address,
	    bool fixed_address, unsigned char *data, unsigned data_length, ARM::AXI::Burst burst, uint8_t extension_mask);
	std::shared_ptr<RequestHandle> dma_internal(uint32_t request_id, uint8_t src_fetch_module,
	                                            uint8_t src_target_module, uint8_t fetch_chiplet, uint8_t fetch_module,
	                                            uint8_t target_chiplet, uint8_t target_module, uint32_t fetch_addr,
  ...
  private:
	...
	struct DelayModel {
	  ...
	  public:
		...
		sc_time irq_transfer(tlm_generic_payload &transaction) const {
			SC_LOG_DELAY(&module, "IRQ Transfer", module.irq_hw_delay);
			return module.irq_hw_delay;
		}
	};
	...
	// Transport Functions
	// -------------------------------------------------------
	tlm_sync_enum nb_transport_fw_irq(int id, tlm_generic_payload &payload, tlm_phase &phase, sc_time &delay);
