...
struct ChipletModuleInfo {
	...
	bool is_manager() const {
		return std::find(types.begin(), types.end(), AXIModuleType::MANAGER) != types.end();
	}

	bool is_subordinate() const {
		return std::find(types.begin(), types.end(), AXIModuleType::SUBORDINATE) != types.end();
	}

	bool is_bus() const {
		return std::find(types.begin(), types.end(), AXIModuleType::BUS) != types.end();
	}

	bool is_bus_manager() const {
		return std::find(types.begin(), types.end(), AXIModuleType::BUS_MANAGER) != types.end();
	}

	bool is_bus_subordinate() const {
		return std::find(types.begin(), types.end(), AXIModuleType::BUS_SUBORDINATE) != types.end();
	}

	bool is_interconnect() const {
		return std::find(types.begin(), types.end(), AXIModuleType::INTERCONNECT) != types.end();
	}
};
...
struct ChipletDescriptor {
	...
	unsigned allocate_module_id() {
		return next_module_id++;
	}

	unsigned allocate_mgr_port() {
		return next_mgr_port++;
	}

	unsigned allocate_sub_port() {
		return next_sub_port++;
	}

	ChipletModuleInfo &add_module(const std::string &name, std::initializer_list<AXIModuleType> types) {
	...
	size_t num_managers() const {
		return std::count_if(modules.begin(), modules.end(), [](const auto &m) { return m.is_manager(); });
	}

	size_t num_subordinates() const {
		return std::count_if(modules.begin(), modules.end(), [](const auto &m) { return m.is_subordinate(); });
	}

	size_t num_bus() const {
		return std::count_if(modules.begin(), modules.end(), [](const auto &m) { return m.is_bus(); });
	}

	size_t num_bus_managers() const {
		return std::count_if(modules.begin(), modules.end(), [](const auto &m) { return m.is_bus_manager(); });
	}

	size_t num_bus_subordinates() const {
		return std::count_if(modules.begin(), modules.end(), [](const auto &m) { return m.is_bus_subordinate(); });
	}

	size_t num_interconnects() const {
		return std::count_if(modules.begin(), modules.end(), [](const auto &m) { return m.is_interconnect(); });
	}
};