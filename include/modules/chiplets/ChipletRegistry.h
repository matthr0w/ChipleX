...
private:
  std::unordered_map<std::string, std::shared_ptr<ChipletDescriptor>>
      chiplets_by_name_;
  std::unordered_map<unsigned, std::shared_ptr<ChipletDescriptor>>
      chiplets_by_id_;
...
public:
  ...
  void register_chiplet(unsigned id, const std::string &name,
