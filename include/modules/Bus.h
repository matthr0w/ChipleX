...
SC_MODULE(Bus) {
  ...
  public:
	...
	// Sockets
	// -------------------------------------------------------
	std::vector<std::unique_ptr<ARM::AXI4::SimpleTargetSocketTagged<Bus>>>    managers;
	std::vector<std::unique_ptr<ARM::AXI4::SimpleInitiatorSocketTagged<Bus>>> subordinates;

	Bus(sc_module_name name, unsigned chiplet_id, ChipletConfig chiplet_config, unsigned num_managers,
  ...
  private:
	...
	// Transport Functions
	// -------------------------------------------------------
	tlm_sync_enum nb_transport_fw(int mgr_id, ARM::AXI::Payload &payload, ARM::AXI::Phase &phase);

	tlm_sync_enum nb_transport_bw(int sub_id, ARM::AXI::Payload &payload, ARM::AXI::Phase &phase);

	// -------------------------------------------------------
	// Debug Functions
	// -------------------------------------------------------
	void print_payload(ARM::AXI::Payload & payload, ARM::AXI::Phase sent_phase, ARM::AXI::Phase reply_phase);
};