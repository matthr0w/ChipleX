...
SC_MODULE(SLDataLinkLayer) {
  ...
  public:
    ...
    SLDataLinkLayer(sc_module_name name, unsigned chiplet_id, unsigned interconnect_id,
  ...
  private:
    ...
    // Transport Functions
    // -------------------------------------------------------
    tlm_sync_enum nb_transport_fw(int id, tlm_generic_payload &transaction, tlm_phase &phase, sc_time &delay);
    tlm_sync_enum nb_transport_bw(int id, tlm_generic_payload &transaction, tlm_phase &phase, sc_time &delay);
