...
SC_MODULE(SLChannelAllocater) {
  ...
  public:
	...
	// Sockets
	// -------------------------------------------------------
	simple_target_socket_tagged<SLChannelAllocater>    data_in_tsocket; // tagged for InterconnectBase
	simple_initiator_socket<SLChannelAllocater>        data_in_isocket;
	simple_target_socket<SLChannelAllocater>           data_out_tsocket;
	simple_initiator_socket_tagged<SLChannelAllocater> data_out_isocket; // tagged for InterconnectBase

	SLChannelAllocater(sc_module_name name, unsigned link_id, InterconnectConfig interconnect_config,
  ...
  private:
	...
	// Transport Functions
	// -------------------------------------------------------
	tlm_sync_enum nb_transport_fw_data_in(int id, tlm_generic_payload &transaction, tlm_phase &phase, sc_time &delay);
	tlm_sync_enum nb_transport_bw_data_in(tlm_generic_payload & transaction, tlm_phase & phase, sc_time & delay);
	tlm_sync_enum nb_transport_fw_data_out(tlm_generic_payload & transaction, tlm_phase & phase, sc_time & delay);
	tlm_sync_enum nb_transport_bw_data_out(int id, tlm_generic_payload &transaction, tlm_phase &phase, sc_time &delay);
};