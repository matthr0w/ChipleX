...
struct PayloadKey {
  ...
  bool operator==(const PayloadKey &other) const {
    return request_id == other.request_id && core_id == other.core_id &&
           source_id == other.source_id;
  }
};
struct PayloadKeyHash {
  std::size_t operator()(const PayloadKey &k) const {
    return ((std::hash<unsigned>()(k.request_id) ^
             (std::hash<unsigned>()(k.core_id) << 1)) >>
            1) ^
...
enum AxiChannel : uint8_t { None = 0, AW = 1, W = 2, AR = 3, R = 4, B = 5 };
...
struct Flit {
  ...
  static constexpr size_t header_size() {
    return sizeof(AxiChannel) + sizeof(uint8_t) + sizeof(ARM::AXI::Burst) +
