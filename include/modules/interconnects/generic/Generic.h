...
SC_MODULE(GenericInterconnect), public InterconnectBase, public DMAForwardInterface {
  private:
  ...
  public:
	...
	GenericInterconnect(sc_module_name name, unsigned chiplet_id, ChipletConfig chiplet_config,
	                    unsigned interconnect_id, InterconnectConfig interconnect_config, DMAEngine *dma_engine);
	~GenericInterconnect();
  ...
  private:
	...
	std::unordered_map<PayloadKey, ARM::AXI::Payload *, PayloadKeyHash> subordinate_payloads;
	std::unordered_map<PayloadKey, ARM::AXI::Payload *, PayloadKeyHash> manager_payloads;
	...
	struct DelayModel {
	  ...
	  public:
		...
		sc_time transfer_delay(int id, tlm_generic_payload &transaction) const {
			...
			flit_transfer_delay =
			    sc_time(static_cast<double>(module.flit_size) / config["phy"]["bandwidth"].as<double>(), SC_NS);
			...
			double prob_bad_transfer = 1.0 - std::pow(1.0 - scaled_ber, module.flit_size * 8);

			int max_attempts = 1;
			switch (module.interconnect_type.value) {
			case InterconnectType::Type::UCIe:
				max_attempts = config["protocol"]["retries"].as<unsigned>() + 1; // + 1 for first try
				break;
			default:
				break;
			}
	...
	tlm_sync_enum nb_transport_fw_phy(int id, tlm_generic_payload &transaction, tlm_phase &phase, sc_time &delay);

	tlm_sync_enum nb_transport_bw_phy(int id, tlm_generic_payload &transaction, tlm_phase &phase, sc_time &delay);
	...
	void write_flit_to_buffer(uint8_t *dest, const Flit &flit, size_t flit_payload_bytes, size_t flit_padding_bytes);
	...
	void erase_payload(std::unordered_map<PayloadKey, ARM::AXI::Payload *, PayloadKeyHash> & payload_map,
