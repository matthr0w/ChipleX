...
class SetupLoader {
  public:
	SetupLoader(const std::string &configs_path, const std::string &setups_path, const std::string &setup_name) {
	...
	const SystemConfig &get_config() const {
		return sysconf_;
	}
  ...
  private:
	...
	void merge_nodes(YAML::Node target, const YAML::Node &override, const std::string &path = "");
};