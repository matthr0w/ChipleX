...
public:
  ...
  // sockets
  // -------------------------------------------------------
  simple_target_socket<Cache> generator_target_socket;
  simple_initiator_socket<Cache> bus_initiator_socket;

  Cache(sc_module_name name, unsigned int fpga_id);
...
private:
  ...
  const sc_time access_delay = fpga_config.get<sc_time>("cache.access_delay");
