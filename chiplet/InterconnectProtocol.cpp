...
InterconnectProtocol::InterconnectProtocol(sc_module_name name,
  ...
  interconnect0_target_socket.register_nb_transport_fw(
      this, &InterconnectProtocol::nb_transport_fw_interconnect0);
  interconnect0_initiator_socket.register_nb_transport_bw(
      this, &InterconnectProtocol::nb_transport_bw_interconnect);

  interconnect1_target_socket.register_nb_transport_fw(
      this, &InterconnectProtocol::nb_transport_fw_interconnect1);
...
void InterconnectProtocol::process_tx_buffer() {
  while (true) {
...
void InterconnectProtocol::process_rx_buffer() {
  while (true) {
...
void InterconnectProtocol::set_write_address(tlm_generic_payload &transaction) {
...
void InterconnectProtocol::send_to_interconnect(
  ...
  auto &socket = (route == 0) ? interconnect0_initiator_socket
                              : interconnect1_initiator_socket;

  SC_LOG_DEBUG(this, *transaction_copy,
               "Protocol->Interconnect" << route << " transmission started");

  tlm_resp = socket->nb_transport_fw(*transaction_copy, phase, delay);
...
tlm_sync_enum InterconnectProtocol::nb_transport_fw_interconnect0(
    tlm_generic_payload &transaction, tlm_phase &phase, sc_time &delay) {
  SC_LOG_DEBUG(this, transaction, "Received request from Interconnect0");
...
tlm_sync_enum InterconnectProtocol::nb_transport_fw_interconnect1(
    tlm_generic_payload &transaction, tlm_phase &phase, sc_time &delay) {
  SC_LOG_DEBUG(this, transaction, "Received request from Interconnect1");
...
// helper functions
void InterconnectProtocol::output_buffer_levels() {
