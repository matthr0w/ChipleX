	...
	static ModuleCodeMap code = {
	     ...
	     {CPUCode{.main =
	                  [](Core &core) {
		                  ...
		                  unsigned char *input_image =
		                      stbi_load(("setups/duckcam/data/duckiebot_input_" + RESOLUTION + ".jpg").c_str(), &width,
		                                &height, &channels, 3);
		                  ...
		                  while (true) {
			                  ...
			                  while (offset < buffer_size) {
				                  ...
				                  auto reqw = AxiRequest(req_id, buffer + offset, chunk_size)
	              ...
	              .irq =
	                  [](Core &core, const IRQ &irq) {
		                  const unsigned  interrupt_num   = RESOLUTION == "160x120" ? 13 : 1;
		                  ...
		                  std::string filename = "setups/duckcam/data/duckiebot_output" + std::to_string(pass) + ".jpg";

		                  stbi_write_jpg(filename.c_str(), header->width, header->height, header->channels, img_data,
		                                 100);
	    ...
	    {{"chiplet0", "core0"},
	     {CPUCode{.main = [](Core &core) {},
	              .irq =
	                  [](Core &core, const IRQ &irq) {
		                  const unsigned  interrupt_num   = RESOLUTION == "160x120" ? 57 : 3;
		                  static unsigned interrupt_count = 0;
		                  static uint32_t base_addr       = 0;
		                  static size_t   total_len       = 0;
		                  ...
		                  for (uint32_t y = 0; y < new_height; ++y) {
			                  std::memcpy(dst + y * new_width * 3, src + (y + crop_margin) * new_width * 3,
			                              new_width * 3);
		                  }
	    ...
	    {{"chiplet1", "core0"},
	     {CPUCode{.main = [](Core &core) {},
	              .irq =
	                  [](Core &core, const IRQ &irq) {
		                  const unsigned  interrupt_num   = RESOLUTION == "160x120" ? 38 : 2;
		                  static unsigned interrupt_count = 0;
		                  static uint32_t base_addr       = 0;
		                  static size_t   total_len       = 0;
