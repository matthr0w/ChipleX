	     ...
	     {CPUCode{.main =
	                  [](Core &core) {
		                  ...
		                  unsigned char *input_img =
		                      stbi_load("setups/tumlogo/data/tum_input.jpg", &width, &height, &channels, 3);
		                  ...
		                  ImageHeader *header = reinterpret_cast<ImageHeader *>(write_buf);
		                  header->width       = width;
	              ...
	              .irq =
	                  [](Core &core, const IRQ &irq) {
		                  ...
		                  ImageHeader *header   = reinterpret_cast<ImageHeader *>(read_buf);
		                  auto        *img_data = read_buf + sizeof(ImageHeader);

		                  stbi_write_jpg("setups/tumlogo/data/tum_output.jpg", header->width, header->height, 3,
		                                 img_data, 100);
	              ...
	              .irq =
	                  [](Core &core, const IRQ &irq) {
		                  ...
		                  for (uint32_t y = 0; y < new_height; ++y) {
			                  unsigned char *src_row = src + ((y + crop_margin) * header->width + crop_margin) * 3;
