	         ...
	         .irq =
	             [](Core &core, const IRQ &irq) {
		             ...
		             case Operation::FetchFromMem: {
			             ...
			             auto dma = AxiDMARequest(2, CHUNK_SIZE) // chunk by chunk
			                            .from_via("mem_chiplet1", "memory", 0x0 + offset, "spi")
	              ...
	              .irq =
	                  [](Core &core, const IRQ &irq) {
		                  ...
		                  case Operation::TransToMem: {
			                  ...
			                  auto dma = AxiDMARequest(10, irq.data_length)
			                                 .from("chiplet2", "memory", irq.target_address)
			                                 .to_via("mem_chiplet2", "memory", 0x0 + offset, "spi");
			                  core.dma(dma);
			                  total_buffer_size -= CHUNK_SIZE;
			                  offset            += CHUNK_SIZE;
		                  } break;
