...
void Router::build_table() {
	...
	for (const auto &conn : sysconf_.connections) {
		adj[conn.endpoint0.chiplet_id].push_back(
		    {conn.endpoint1.chiplet_id, conn.endpoint0.interconnect_id, conn.endpoint0.link_id});

		adj[conn.endpoint1.chiplet_id].push_back(
		    {conn.endpoint0.chiplet_id, conn.endpoint1.interconnect_id, conn.endpoint1.link_id});

		dest_table_[encode_link(conn.endpoint0.chiplet_id, conn.endpoint0.interconnect_id, conn.endpoint0.link_id)] =
		    conn.endpoint1.chiplet_id;
		dest_table_[encode_link(conn.endpoint1.chiplet_id, conn.endpoint1.interconnect_id, conn.endpoint1.link_id)] =
		    conn.endpoint0.chiplet_id;
	}
	...
	for (int src = 0; src < num_chiplets; ++src) {
		...
		while (!q.empty()) {
			...
			for (auto &[neighbor, interconnect_id, link_id] : adj[current]) {
				...
				int hop_interconnect = (current == src) ? interconnect_id : first_hop_interconnect;
