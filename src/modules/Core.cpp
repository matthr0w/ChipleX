...
Core::Core(sc_module_name name, const std::string &chiplet_name, unsigned chiplet_id, const std::string &core_name,
	...
	for (unsigned i = 0; i < num_irqs; ++i) {
		irq_sockets[i].register_nb_transport_fw(this, &Core::nb_transport_fw_irq, i);
	}
...
// Transport Functions
// -------------------------------------------------------
tlm_sync_enum Core::nb_transport_fw_irq(int id, tlm_generic_payload &transaction, tlm_phase &phase,
...
tlm_sync_enum Core::nb_transport_bw(ARM::AXI::Payload &payload, ARM::AXI::Phase &phase) {
	switch (phase) {
...
std::shared_ptr<RequestHandle> Core::read_internal(uint32_t request_id, uint8_t src_module, uint8_t dst_chiplet,
	...
	ARM::AXI::Payload *payload = ARM::AXI::Payload::new_payload(ARM::AXI::COMMAND_READ, address, size, len, burst);
...
std::shared_ptr<RequestHandle> Core::read(const AxiRequest &req) {
	...
	SC_LOG_ASSERT(this, req.data_length <= max_burst,
	              "AXI Request Error: Requested data length ("
	                  << req.data_length << " bytes) exceeds maximum allowed burst size (" << max_burst << " bytes)");

	auto        chiplet_desc     = ChipletRegistry::instance().get(chiplet_id);
	std::string src_chiplet_name = chiplet_desc->chiplet_name;
	std::string dst_chiplet_name = req.dst_chiplet_name.value_or(src_chiplet_name);
	...
	SC_LOG_ASSERT(this, dst_chiplet_desc, "AXI Request Error: Chiplet " << dst_chiplet_name << " does not exist");

	uint8_t dst_chiplet_id = dst_chiplet_desc->chiplet_id;

	auto *src_module = ChipletRegistry::instance().get_module(chiplet_id, src_module_name);

	auto *dst_module = ChipletRegistry::instance().get_module(dst_chiplet_id, dst_module_name);

	SC_LOG_ASSERT(this, src_module,
	              "AXI Request Error: Module " << src_module_name << " does not exist on " << src_chiplet_name);

	SC_LOG_ASSERT(this, dst_module,
	              "AXI Request Error: Module " << dst_module_name << " does not exist on " << dst_chiplet_name);

	SC_LOG_ASSERT(this, src_module->is_subordinate(),
	              "AXI Request Error: Module " << src_module_name << " is not an AXI subordinate");

	SC_LOG_ASSERT(this, dst_module->is_subordinate(),
	              "AXI Request Error: Module " << dst_module_name << " is not an AXI subordinate");
	...
	SC_LOG_ASSERT(this, req.address.has_value(), "AXI Request Error: Read requests require an address");
...
std::shared_ptr<RequestHandle> Core::write_internal(uint32_t request_id, uint8_t src_module, uint8_t dst_chiplet,
	...
	ARM::AXI::Payload *payload = ARM::AXI::Payload::new_payload(ARM::AXI::COMMAND_WRITE, address, size, len, burst);
...
std::shared_ptr<RequestHandle> Core::dma(const AxiDMARequest &req) {
	...
	SC_LOG_ASSERT(this, fetch_chiplet_desc, "AXI Request Error: Chiplet " << fetch_chiplet_name << " does not exist");
	...
	auto target_chiplet_desc = ChipletRegistry::instance().get(target_chiplet_name);

	SC_LOG_ASSERT(this, target_chiplet_desc, "AXI Request Error: Chiplet " << target_chiplet_name << " does not exist");

	uint8_t target_chiplet_id = target_chiplet_desc->chiplet_id;

	if (fetch_chiplet_id != chiplet_id) {
		SC_LOG_ASSERT(this, req.src_fetch_module_name.has_value(),
		              "AXI DMA Request Error: Source fetch module not set. Did you call "
		              "from_via() method?");
	}

	if (target_chiplet_id != chiplet_id) {
		SC_LOG_ASSERT(this, req.src_target_module_name.has_value(),
	...
	auto *src_fetch_module  = ChipletRegistry::instance().get_module(chiplet_id, src_fetch_module_name);
	auto *src_target_module = ChipletRegistry::instance().get_module(chiplet_id, src_target_module_name);

	SC_LOG_ASSERT(this, src_fetch_module,
	              "AXI DMA Request Error: Module " << src_fetch_module_name << " does not exist on "
	                                               << src_chiplet_name);
	SC_LOG_ASSERT(this, src_target_module,
	              "AXI DMA Request Error: Module " << src_target_module_name << " does not exist on "
	                                               << src_chiplet_name);

	auto *fetch_module = ChipletRegistry::instance().get_module(fetch_chiplet_id, fetch_module_name);

	SC_LOG_ASSERT(this, fetch_module,
	              "AXI DMA Request Error: Module " << fetch_module_name << " does not exist on " << fetch_chiplet_name);

	auto *target_module = ChipletRegistry::instance().get_module(target_chiplet_id, target_module_name);

	SC_LOG_ASSERT(this, target_module,
	              "AXI DMA Request Error: Module " << target_module_name << " does not exist on "
	                                               << target_chiplet_name);

	SC_LOG_ASSERT(this, src_fetch_module->is_subordinate(),
	              "AXI DMA Request Error: Module " << src_fetch_module_name << " is not an AXI subordinate");

	SC_LOG_ASSERT(this, src_target_module->is_subordinate(),
	              "AXI DMA Request Error: Module " << src_target_module_name << " is not an AXI subordinate");

	SC_LOG_ASSERT(this, fetch_module->is_subordinate(),
	              "AXI DMA Request Error: Module " << fetch_module_name << " is not an AXI subordinate");
