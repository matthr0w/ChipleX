...
Cache::Cache(sc_module_name name, unsigned chiplet_id, ChipletConfig chiplet_config)
    : sc_module(name),
      ...
      cache_block_size(chiplet_config.node["caches"]["block_size"].as<unsigned>()),
      cache_store_buffer_size(chiplet_config.node["caches"]["store_buffer_size"].as<unsigned>()),
      clk_cycle(chiplet_config.node["caches"]["clk_cycle"].as<unsigned>(), SC_NS),
      beat_data(new uint8_t[axi_width >> 3]),
      tsocket("tsocket", *this, &Cache::nb_transport_fw, ARM::TLM::PROTOCOL_AXI4, axi_width),
      isocket("isocket", *this, &Cache::nb_transport_bw, ARM::TLM::PROTOCOL_AXI4, axi_width) {
	// Assertions
	LOG_ASSERT(cache_size % cache_block_size == 0,
	...
	LOG_ASSERT(is_power_of_two(cache_block_size), "Parameter Error: Cache block_size must be a power of two");
	LOG_ASSERT(is_power_of_two(num_lines), "Parameter Error: Cache line count (size / block_size) must be a "
	                                       "power of two");
	LOG_ASSERT(cache_store_buffer_size >= 2, "Parameter Error: Cache store_buffer_size must be at least 2 (the "
...
void Cache::end_of_simulation() {
	double hit_rate  = num_accesses > 0 ? double(100) * num_hits / num_accesses : 0;
	double miss_rate = num_accesses > 0 ? double(100) * num_misses / num_accesses : 0;
...
void Cache::clk_posedge() {
	...
	if (b_outgoing && !w_queue_in.empty()) {
		...
		if (next_tail != store_buffer_head) {
			uint32_t beat_addr =
			    set_beat_address(b_outgoing->get_address(), w_beat_count_in, b_outgoing->get_beat_data_length(),
			                     b_outgoing->get_beat_count(), b_outgoing->get_burst());
			...
			while (remaining > 0) {
				...
				uint32_t tag   = block_address >> (log2_pow2(cache_block_size) + log2_pow2(num_lines));
				...
				if (!(line.valid && line.tag == tag)) {
					...
					// Write Hit
					SC_LOG_DEBUG(this, "Write Hit: Writing data to store buffer and cache");
					num_hits++;

					std::memcpy(&line.data[block_offset], sbe.data.data() + beat_offset, copy_len);
				}
	...
	if (r_outgoing && active_cache_line_requests.empty()) {
		uint32_t beat_addr =
		    set_beat_address(r_outgoing->get_address(), r_beat_count, r_outgoing->get_beat_data_length(),
		...
		while (remaining > 0) {
			...
			if (!(line.valid && line.tag == tag)) {
				...
				SC_LOG_DEBUG(this, "Read Miss: Enqueuing line fetch from address 0x" << std::hex << block_address);
				num_misses++;
				...
				std::memcpy(beat_data + beat_offset, &line.data[block_offset], copy_len);
			}
...
void Cache::clk_negedge() {
	...
	if (w_state == CLEAR && !w_queue_out.empty()) {
		...
		ARM::AXI::Phase    phase =
		    (w_beat_count_out + 1 == payload->get_beat_count()) ? ARM::AXI::W_VALID_LAST : ARM::AXI::W_VALID;
	...
	if (r_beat_ready) {
		ARM::AXI::Phase phase =
		    (r_beat_count + 1 == r_outgoing->get_beat_count()) ? ARM::AXI::R_VALID_LAST : ARM::AXI::R_VALID;
...
// Transport Functions
// -------------------------------------------------------
tlm_sync_enum Cache::nb_transport_fw(ARM::AXI::Payload &payload, ARM::AXI::Phase &phase) {
...
tlm_sync_enum Cache::nb_transport_bw(ARM::AXI::Payload &payload, ARM::AXI::Phase &phase) {
	// Skip cache if AXI signal not set
	if (payload.cache != ARM::AXI::CACHE_AR_WRITE_THROUGH_RWA) {
...
// Helper Functions
// -------------------------------------------------------
uint32_t Cache::set_beat_address(uint32_t base, unsigned beat_idx, unsigned beat_bytes, unsigned beats,
                                 ARM::AXI::Burst burst) {
	switch (burst) {
...
void Cache::enqueue_cacheline_read() {
	...
	ARM::AXI::Payload *payload =
	    ARM::AXI::Payload::new_payload(ARM::AXI::COMMAND_READ, clr.address, size, len, ARM::AXI::BURST_INCR);

	UserSignals user;
	user.src_chiplet   = chiplet_id;
	user.dst_chiplet   = chiplet_id;
	user.src_module    = ChipletRegistry::instance().get_module(chiplet_id, "memory")->id;
...
void Cache::enqueue_storebuffer_write() {
	...
	ARM::AXI::Payload *payload =
	    ARM::AXI::Payload::new_payload(ARM::AXI::COMMAND_WRITE, sbe.address, size, len, ARM::AXI::BURST_INCR);
...
void Cache::dump() {
	...
	for (size_t i = 0; i < cache_lines.size(); ++i) {
		...
		std::cout << "Line " << i << " [valid=" << line.valid << ", tag=" << std::setw(2) << line.tag << "]: ";
