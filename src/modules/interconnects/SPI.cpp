...
SPI::SPI(sc_module_name name, unsigned chiplet_id, ChipletConfig chiplet_config, unsigned interconnect_id,
    ...
    : InterconnectBase(chiplet_id, chiplet_config, interconnect_id, interconnect_config),
      ...
      axi_in("axi_in", *this, &SPI::nb_transport_fw_axi, ARM::TLM::PROTOCOL_AXI4, axi_width),
      axi_out("axi_out", *this, &SPI::nb_transport_bw_axi, ARM::TLM::PROTOCOL_AXI4, axi_width) {
	if (dma_engine) {
...
void SPI::end_of_simulation() {
	for (size_t id = 0; id < connections.size(); ++id) {
		...
		stats.set_value(this->name(), "transmission_size_bits_link" + std::to_string(id), axi_width);
		stats.set_value(this->name(), "efficiency_pJ_bit_link" + std::to_string(id), config["efficiency"].as<double>());
	}
}

void SPI::bind_clocks(Clocks &clocks) {
	clk.bind(clocks.get());
}
...
void SPI::clk_posedge() {
	...
	// 2. For AXI beats not to the bus, arbitrate in the following order:
...
// Transport Functions
// -------------------------------------------------------
tlm_sync_enum SPI::nb_transport_fw_axi(ARM::AXI::Payload &payload, ARM::AXI::Phase &phase) {
	switch (phase) {
...
tlm_sync_enum SPI::nb_transport_bw_axi(ARM::AXI::Payload &payload, ARM::AXI::Phase &phase) {
	switch (phase) {
...
tlm_sync_enum SPI::nb_transport_fw_link(int id, tlm_generic_payload &transaction, tlm_phase &phase, sc_time &delay) {
	...
	case BEGIN_REQ:
		...
		sc_spawn([this, id, &transaction, delay]() {
			...
			ARM::AXI::Payload *payload = reinterpret_cast<ARM::AXI::Payload *>(transaction.get_data_ptr());
...
tlm_sync_enum SPI::nb_transport_bw_link(int id, tlm_generic_payload &transaction, tlm_phase &phase, sc_time &delay) {
	switch (phase) {
	case BEGIN_RESP:
		stats.set_idle(this->name());
