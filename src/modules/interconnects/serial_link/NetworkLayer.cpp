...
SLNetworkLayer::SLNetworkLayer(sc_module_name name, unsigned chiplet_id, unsigned interconnect_id,
	...
	sensitive << update_event << aw_gnt << ar_gnt << axi_in_sig << axi_out_sig << committer_state_q;

	SC_THREAD(sender_thread);
	sensitive << update_event << aw_gnt << w_gnt << b_gnt << ar_gnt << r_gnt << axis_reg_valid_in << axis_reg_ready_in;
}

SLNetworkLayer::~SLNetworkLayer() {
	delete[] irq_sockets;
}
...
void SLNetworkLayer::sender_thread() {
	while (true) {
		...
		if (aw_gnt.read()) {
			...
			payload_out->axi_ch.addr = axi_in_trans.w_payload->get_address();
			if (axi_in_trans.w_beat_count < axi_in_trans.w_payload->get_beats_complete()) {
				axi_in_trans.w_payload->write_out_beat(axi_in_trans.w_beat_count, payload_out->axi_ch.data.data());
			}
			payload_out->id    = axi_in_trans.w_payload->id;
			payload_out->len   = axi_in_trans.w_payload->get_beat_count() - 1;
			payload_out->burst = axi_in_trans.w_payload->get_burst();
			...
			if (axi_out_trans.r_beat_count < axi_out_trans.r_payload->get_beats_complete()) {
				axi_out_trans.r_payload->read_out_beat(axi_out_trans.r_beat_count, payload_out->axi_ch.data.data());
			}
		...
		if (payload_out->hdr == TagIdle) {
			...
			int destination_id = Router::instance().get_dest_id(chiplet_id, interconnect_id, link_id);
			if (destination_id == -1) {
				SC_LOG_ERROR(this, "No valid routing path from " << chiplet_id << " via link" << int(link_id));
			}
...
tlm_sync_enum SLNetworkLayer::nb_transport_bw(ARM::AXI::Payload &payload, ARM::AXI::Phase &phase) {
	switch (phase) {
...
void SLNetworkLayer::send_axi_response(AxiTrans_t &trans, bool is_master) {
	auto send = [&](bool &rsp_sent, ARM::AXI::Phase &rsp_phase, ARM::AXI::Phase &req_phase) {
		if (rsp_sent) {
			return;
		}
