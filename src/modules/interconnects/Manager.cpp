...
std::unique_ptr<InterconnectBase> InterconnectManager::create_interconnect(
    const std::string name, const unsigned interconnect_id,
    const InterconnectConfig interconnect_config, DMAEngine *dma_engine) {
  ...
  case InterconnectType::Type::UCIe:
    return std::make_unique<GenericInterconnect>(
        sc_module_name(name.c_str()), chiplet_id, chiplet_config,
        interconnect_id, interconnect_config, dma_engine);
  case InterconnectType::Type::SerialLink:
    return std::make_unique<SerialLink>(sc_module_name(name.c_str()),
                                        chiplet_id, chiplet_config,
                                        interconnect_id, interconnect_config);
  case InterconnectType::Type::SPI:
    return std::make_unique<SPI>(sc_module_name(name.c_str()), chiplet_id,
                                 chiplet_config, interconnect_id,
