...
ExtensionLayer::ExtensionLayer(sc_module_name name, ChipletConfig chiplet_config, DMAEngine *dma_engine)
    : sc_module(name),
      num_cores(chiplet_config.node["cores"]["num"].as<unsigned>()),
      axi_width(chiplet_config.node["axi"]["width"].as<unsigned>()),
      dma_engine(dma_engine),
      axi_in_up("axi_in_up", *this, &ExtensionLayer::nb_transport_fw_up, ARM::TLM::PROTOCOL_AXI4, axi_width),
      axi_out_up("axi_out_up", *this, &ExtensionLayer::nb_transport_bw_up, ARM::TLM::PROTOCOL_AXI4, axi_width),
      axi_in_down("axi_in_down", *this, &ExtensionLayer::nb_transport_fw_down, ARM::TLM::PROTOCOL_AXI4, axi_width),
      axi_out_down("axi_out_down", *this, &ExtensionLayer::nb_transport_bw_down, ARM::TLM::PROTOCOL_AXI4, axi_width) {
	// Create IRQ sockets
	...
	extensions[SmartExtension::CRYPTO] = std::make_unique<CryptoExtension>(axi_width);
...
// Transport Functions
// -------------------------------------------------------
tlm_sync_enum ExtensionLayer::nb_transport_fw_up(ARM::AXI::Payload &payload, ARM::AXI::Phase &phase) {
	...
	case ARM::AXI::AW_VALID:
		...
		up.aw_in.push_back({&payload, ARM::AXI::AW_VALID, AxiDir::DOWNSTREAM, payload_beat_index[&payload]});
		break;
	case ARM::AXI::W_VALID:
		up.w_in.push_back({&payload, ARM::AXI::W_VALID, AxiDir::DOWNSTREAM, ++payload_beat_index[&payload]});
		break;
	case ARM::AXI::W_VALID_LAST:
		up.w_in.push_back({&payload, ARM::AXI::W_VALID_LAST, AxiDir::DOWNSTREAM, ++payload_beat_index[&payload]});
		break;
	...
	case ARM::AXI::AR_VALID:
		payload_beat_index[&payload] = -1;
		up.ar_in.push_back({&payload, ARM::AXI::AR_VALID, AxiDir::DOWNSTREAM, payload_beat_index[&payload]});
		break;
...
tlm_sync_enum ExtensionLayer::nb_transport_bw_up(ARM::AXI::Payload &payload, ARM::AXI::Phase &phase) {
	UserSignals user = UserSignals::decode(payload.user);
	...
	case ARM::AXI::B_VALID:
		up.b_in.push_back({&payload, ARM::AXI::B_VALID, AxiDir::DOWNSTREAM, payload_beat_index[&payload]});
		send_irq(payload, 0);
		break;
	...
	case ARM::AXI::R_VALID:
		up.r_in.push_back({&payload, ARM::AXI::R_VALID, AxiDir::DOWNSTREAM, ++payload_beat_index[&payload]});
		break;
	case ARM::AXI::R_VALID_LAST:
		up.r_in.push_back({&payload, ARM::AXI::R_VALID_LAST, AxiDir::DOWNSTREAM, ++payload_beat_index[&payload]});
		break;
	default:
		SC_LOG_ERROR(this, "AXI TLM Protocol: Unexpected phase: " << get_axi_phase_string(phase));
	}
...
tlm_sync_enum ExtensionLayer::nb_transport_fw_down(ARM::AXI::Payload &payload, ARM::AXI::Phase &phase) {
	UserSignals user = UserSignals::decode(payload.user);
	...
	case ARM::AXI::AW_VALID:
		payload_beat_index[&payload] = -1;
		down.aw_in.push_back({&payload, ARM::AXI::AW_VALID, AxiDir::UPSTREAM, payload_beat_index[&payload]});
		break;
	case ARM::AXI::W_VALID:
		down.w_in.push_back({&payload, ARM::AXI::W_VALID, AxiDir::UPSTREAM, ++payload_beat_index[&payload]});
		break;
	case ARM::AXI::W_VALID_LAST:
		down.w_in.push_back({&payload, ARM::AXI::W_VALID_LAST, AxiDir::UPSTREAM, ++payload_beat_index[&payload]});
		break;
	case ARM::AXI::B_READY:
		payload_beat_index.erase(&payload);
	...
	case ARM::AXI::AR_VALID:
		payload_beat_index[&payload] = -1;
		down.ar_in.push_back({&payload, ARM::AXI::AR_VALID, AxiDir::UPSTREAM, payload_beat_index[&payload]});
		break;
	case ARM::AXI::R_READY:
		if (payload.get_beats_complete() == payload_beat_index[&payload] + 1) {
			payload_beat_index.erase(&payload);
...
tlm_sync_enum ExtensionLayer::nb_transport_bw_down(ARM::AXI::Payload &payload, ARM::AXI::Phase &phase) {
	UserSignals user = UserSignals::decode(payload.user);
	...
	case ARM::AXI::W_READY:
		...
		break;
	case ARM::AXI::B_VALID:
		down.b_in.push_back({&payload, ARM::AXI::B_VALID, AxiDir::UPSTREAM, payload_beat_index[&payload]});
		break;
	case ARM::AXI::AR_READY:
		down.ar_state = down.ar_state == REQ ? ACK : CLEAR;
		break;
	case ARM::AXI::R_VALID:
		down.r_in.push_back({&payload, ARM::AXI::R_VALID, AxiDir::UPSTREAM, ++payload_beat_index[&payload]});
		break;
	case ARM::AXI::R_VALID_LAST:
		down.r_in.push_back({&payload, ARM::AXI::R_VALID_LAST, AxiDir::UPSTREAM, ++payload_beat_index[&payload]});
		break;
	default:
		SC_LOG_ERROR(this, "AXI TLM Protocol: Unexpected phase: " << get_axi_phase_string(phase));
	}
...
void ExtensionLayer::process_in_queues(AxiSide &side) {
	auto push_ext = [&](std::deque<AxiBeat> &queue, ARM::AXI::Phase ready_phase) {
		...
		ExtensionBase *ext  = select_extension(UserSignals::decode(beat.payload->user).extension_mask);
...
void ExtensionLayer::process_out_queues(AxiSide &side, bool use_dma) {
	auto send_fw = [&](ChannelState &state, std::deque<AxiBeat> &queue) {
		...
		if (use_dma && !send_dma_request(*beat.payload, beat.phase)) {
			state = CLEAR;
		} else if (!use_dma && side.fw_out->nb_transport_fw(*beat.payload, phase) == TLM_UPDATED) {
			state = ACK;
		}
	};
